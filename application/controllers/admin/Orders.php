...
class Orders extends CI_Controller {
	...
	public function status($id)
	{
		...
		if($status == 0)
		{ 
			$val_id = $this->uri->rsegment('3');;
			$deatail = $this->Morderdetail->orderdetail_hoa($val_id);
		// echo "<pre>";
		// print_r($deatail);
		// die();
			$hoaloan = '';
			foreach ($deatail as $loan) {
				$sanpham = $this->Mproduct->layrasanpham($loan['productid']);
				//echo $sanpham[0]['number'];
			//echo "<pre>";
			//print_r($sanpham);
			//die();
				$sosanpham = $sanpham[0]['number'];
				$sosanphambanra = $sanpham[0]['number_buy'];
				$spbanraupdate =  $sosanphambanra + $loan['count'];
				$spudate = $sosanpham - $loan['count'];
				$mydata1 = array('number' => $spudate, 'number_buy' => $spbanraupdate);
				$this->Mproduct->pro_update($mydata1, $loan['productid']);
			}
			$status = 1;
